...
The 1,000 outcomes from 1,000 people playing are independent draws from this urn. If red comes up, the gambler wins and the casino loses a dollar, so we draw a -1 dollar. Otherwise, the casino wins a dollar and we draw a 1 dollar. To construct our random variable $S$, we can use this code:
```{r}
n<-1000
...
mean(S<0)
```
We can visualize the distribution of $S$ by creating a histogram showing the probability $F(a)-F(b)$ for several intervals (a,b].

![histogram](https://rafalab.github.io/dsbook/book_files/figure-html/normal-approximates-distribution-1.png)
...
E[X]=ap+b(1-p)
$$
To see this, notice that if there are n beads in the urn, then we have $npa$s and $n(1−p)b$s and because the average is the sum, $n*a*p+n*b*(1-p)$, To see this, divided by the total n, we get that the average is $ap+b(1-p)$.
