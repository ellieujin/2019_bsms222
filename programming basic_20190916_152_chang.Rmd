...
The formula for the sum of the series 1 to n is n(n+1)/2. What if we weren’t sure that was the right function? How could we check? Using what we learned about functions we can create one that computes the Sn:
```{r}
...
```
In each iteration n=1, n=2, etc…, we computeSn and store it in the  
nth entry of `s_n`.
...
altman_plot(c(5,2,6,7),c(7,3,9,11))
```
...
7. Write a function compute_s_n that for any given n computes the sum Sn=1^2+...+n^2. Report the value of the sum when n=10.
```{r}
s<-vector(length=30)
compute_s_n<-function(n){
  x<-1:n
