...
gapminder %>%
  ...
  select(country, infant_mortality)
```
It turns out that when Hans Rosling gave this quiz to educated groups of people, the average score was less than 2.5 out of 5, worse than what they would have obtained had they guessed randomly. __This implies that more than ignorant, we are misinformed__. In this chapter we see how data visualization helps inform us.
...
# 10.3 Faceting
We could easily plot the 2012 data in the same way we did for 1962. To make comparisons, however, side by side plots are preferable. In _ggplot2_, we can achieve this by faceting variables: we stratify the data by some variable and make the same plot for each strata.
...
In the plot we just made, the regions are ordered alphabetically. But there is no practical reason for this to be the case. In general, __do not order alphabetically__. Data exploration is often made easier if we order the strata by some interpretable quantity. This can be achieved using the `reorder` function, which we covered in Section 3.5.4.
...
Finally, we point out that if what we are most interested is in comparing before and after values, it might make more sense to plot the percentage increases. We are still not ready to learn to code this.
...
In __ggplot2__, we access these variables by surrounding the name with two dots `..`. We will use the following mapping:
